...
| [oldPos](#oldpos)<a name="oldpos"></a>| Vector2[] | | Holds the value of `Projectile.position` from previous updates. Use for drawing trails. Must be used with `ProjectileID.Sets.TrailCacheLength` and `ProjectileID.Sets.TrailingMode` to be used. See [ExampleBullet](https://github.com/tModLoader/tModLoader/blob/1.4.4/ExampleMod/Content/Projectiles/ExampleBullet.cs) |

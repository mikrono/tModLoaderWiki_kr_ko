...
* Open the `client.log` file in a text editor, such as notepad. The `client.log` file is found at `%UserProfile%\Documents\My Games\Terraria\ModLoader\Logs\client.log` (or `[tModLoaderInstallFolder]\tModLoader-Logs\client.log` for 1.4. To get to your install directory on Steam - right click tModLoader in the library, then hover over Manage and click on Browse local files.)

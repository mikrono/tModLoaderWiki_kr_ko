...
### Manually Port
If tModLoader is still unable to migrate files, then a user can do it manually. First, create the `tModLoader-1.4.3` [saves folder](https://github.com/tModLoader/tModLoader/wiki/Basic-tModLoader-Usage-Guide#saves) corresponding to the version you are trying to launch. Next, copy the files and folders from the `tModLoader` folder to the `tModLoader-1.4.3` folder. If this fails, you might have some corrupted files that need to be deleted first.
